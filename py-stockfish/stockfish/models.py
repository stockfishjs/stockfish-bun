...
class Stockfish:
    ...
    def __init__(
        ...
        Example:
            ...
            >>> stockfish = Stockfish()
        """
        self._DEFAULT_STOCKFISH_PARAMS: dict[str, Any] = {
            "Debug Log File": "",
            "Contempt": 0,
            "Min Split Depth": 0,
            "Threads": 1,
    ...
    def update_engine_parameters(self, parameters: dict[str, Any] | None) -> None:
    ...
    def _set_option(self, name: str, value: Any, update_parameters_attribute: bool = True) -> None:
    ...
    def _on_weaker_setting(self) -> bool:
        return self._parameters["UCI_LimitStrength"] or self._parameters["Skill Level"] < 20
    ...
    def set_fen_position(self, fen_position: str, send_ucinewgame_token: bool = True) -> None:
    ...
    def set_position(self, moves: list[str] | None = None) -> None:
        ...
        """
        self.set_fen_position("rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1", True)
    ...
    def set_skill_level(self, skill_level: int = 20) -> None:
        ...
        """
        self.update_engine_parameters({"UCI_LimitStrength": False, "Skill Level": skill_level})
    ...
    def set_elo_rating(self, elo_rating: int = 1350) -> None:
        ...
        """
        self.update_engine_parameters({"UCI_LimitStrength": True, "UCI_Elo": elo_rating})
    ...
    def get_best_move(self, wtime: int | None = None, btime: int | None = None) -> str | None:
    ...
    def get_evaluation(self, searchtime: int | None = None) -> dict[str, str | int]:
        ...
        compare: int = 1 if self.get_turn_perspective() or ("w" in self.get_fen_position()) else -1
    ...
    def get_static_eval(self) -> float | None:
        ...
        compare: int = (
            1 if not self.get_turn_perspective() or ("w" in self.get_fen_position()) else -1
        )
        self._put("eval")
        while True:
            text = self._read_line()
    ...
    def get_top_moves(
        self, num_top_moves: int = 5, verbose: bool = False, num_nodes: int = 0
    ) -> list[dict[str, Any]]:
        ...
        for line in reversed(lines):
            ...
            move_evaluation: dict[str, str | int | None] = {
                ...
                # get cp if available
                "Centipawn": (int(self._pick(line, "cp")) * perspective if "cp" in line else None),
                # get mate if available
                "Mate": (int(self._pick(line, "mate")) * perspective if "mate" in line else None),
            }
    ...
    def get_what_is_on_square(self, square: str) -> "Stockfish.Piece | None":
    ...
    def will_move_be_a_capture(self, move_value: str) -> "Stockfish.Capture":
        ...
        starting_square_piece: Stockfish.Piece | None = self.get_what_is_on_square(move_value[:2])
        ending_square_piece: Stockfish.Piece | None = self.get_what_is_on_square(move_value[2:4])
        if ending_square_piece is not None:
            if not self._parameters["UCI_Chess960"]:
                ...
                if [starting_square_piece, ending_square_piece] in castling_pieces:
                    ...
                    return Stockfish.Capture.DIRECT_CAPTURE
        elif move_value[2:4] == self.get_fen_position().split()[3] and starting_square_piece in [
    ...
    def _parse_stockfish_version(self, version_text: str = "") -> None:
        try:
            ...
            if self._version["text"].startswith("dev-"):
                ...
                self._version["text"] = self._get_stockfish_version_from_build_date(date_string)
    ...
    def _get_stockfish_version_from_build_date(self, date_string: str = "") -> str | None:
    ...
    class BenchmarkParameters:
        ...
        def __post_init__(self):
            ...
            self.evalType = (
                self.evalType if self.evalType in ["mixed", "classical", "NNUE"] else "mixed"
            )
